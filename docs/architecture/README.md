...
| Note | What it covers |
|------|----------------|
| [The poll loop](./poll-loop.md) | Why polling beats the event stream, what one poll does, what survives between polls, how the loop behaves when Herdr is unreachable, and how a restart takes the session over |
