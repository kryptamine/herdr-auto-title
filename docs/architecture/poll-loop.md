...
With `HERDR_AUTO_TITLE_WORKSPACES` on, a poll ends by naming each workspace that
holds exactly one tab. Going last matters only to a poll cut short: the row is
