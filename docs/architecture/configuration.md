...
The file is read once. Half the settings are consumed in `app.New` while it
builds the resolver chain — `MAX_LENGTH`, `BRANCH_MAX`, `POSITION` and `SCROLL`
are baked into `resolver.Default` — so rereading the file mid-run would apply
some settings and quietly ignore others.
